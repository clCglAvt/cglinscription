<?php
/*
 * Copie de document.php de Dolibarr
 *
 * Version CAV 5 avril 2017 
...
/**
 *	\file       htdocs/custom/cglinscription/documentInscLoc.php
 *  \brief      constitution du fichier odt des 4 saisons (bulletin) et location (contrats) et Wrapper to download data files 
...
dol_syslog("document.php download $original_file ");
// Security check
...
if ($id_bull ) 				$bull->fetch_complet_filtre(-1, $id_bull);	

/*
 * Action
 */
 if ($action == 'CreerBulletin') {
	//require_once "./core/modules/cglinscription/modules_cglinscription.php";
	$travail = new CglInscription ($db);
	$linetmp=$bull->RecherchePremLignBySess($session);
	$travail->creer_bulletin($session, $linetmp->activite_lieu);
 }
// None
...
// Security:
// on recherche le nom du bulletin de $bull->ref et $session
	$original_file = $bull->NommageEditionBulletin('fichier', $session);
	
	$original_file .= '.'.odt;
//$original_file = DOL_DATA_ROOT.'/Cglinscription/bulletin/'.$bull->ref.'/'.$bull->ref.'_BULL_IND_'.$session.'.odt';
