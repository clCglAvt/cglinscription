 ...
 *								  - refonte écran ajout colonne Nb Enfant, Nb Adulte et Liste BU du départ - affichage colonne à discretion
 * Version CAV - 2.8.4 - printemps 2023 - Les départs sans participant apparaissent dans la liste des participants
 *							bug 321 - Ajouter les d&parts qui ont des bulletins archivés pour annulation, ils ont besoin d'être annulés. 
 * Version CAV - 2.8.5 - printemps 2023
 *		- absence des bulletin d'un départ si celui-ci n'a pas de moniteur (bug 325)
 *		- tri croissant des départs (evo 333)
 *
*
...
$arrayfields=array(
	...
	'ListBull'=>array('label'=>$langs->trans("ListBull"), 'checked'=>1)
);
...
if (GETPOST("action", 'alpha')=='genere')
{
	...
	function fetch_all( $sortorder, $sortfield, $limit, $offset, $filter = array()) {
		...
		$sql .= "  count(case when bull.statut = 0 then bull.rowid end)  as nbencours ,   ";
		$sql .= "  count(case when (bull.statut < 9  and bull.statut>= 0) then bull.rowid end ) as nbbull,   ";
		$sql .= " sum(case when (bull.statut < 9  and !isnull(bulldet.age) and not(bulldet.age between 0 and 16)) then bulldet.qte else 0 end) as NbAdulte, ";
		$sql .= " sum(case when (bull.statut < 9  and !isnull(bulldet.age) and bulldet.age between 0 and 16) then bulldet.qte else 0 end) as NbEnf, ";
		$sql .= " sum(case when (bull.statut < 9  and isnull(bulldet.age))  then bulldet.qte else 0 end) as NbAgeNonRensg ";
		...
		$sql .= " LEFT JOIN ".MAIN_DB_PREFIX."agefodd_session_extrafields as AgSe ";
		$sql .= " ON s.rowid =  AgSe.fk_object ";
		$sql .= " LEFT JOIN ".MAIN_DB_PREFIX."cglinscription_bull_det as bulldet ";
		$sql .= " ON bulldet.fk_activite =  s.rowid  and bulldet.action not in ('X','S')";
		$sql .= " LEFT JOIN ".MAIN_DB_PREFIX."cglinscription_bull as bull ";
		$sql .= " ON bull.rowid=bulldet.fk_bull ";	
		$sql .= " WHERE s.entity IN (" . getEntity ( 'agsession' ) . ")";
		$sql .= " and s.status not in ( 4,5)";
		//$sql .= ' and ( isnull(bulldet.action) or bulldet.action not in ("X","S")) ';
		$sql .= ' and (isnull(bull.statut) or bull.statut>= '.$bull->BULL_ENCOURS.') ';
		...
		$sql .= " p.ref_interne, c.intitule, s.intitule_custo, c.ref,c.ref_interne,  f.rowid";
		if ( empty($sortfield)) $sql .= " ORDER BY dated ";
		else $sql .= " ORDER BY " . $sortfield . ' ' . $sortorder;
	
		if (! empty ( $limit )) {
			$sql .= ' ' . $agf->db->plimit ( $limit + 1, $offset );
		}
		dol_syslog ( "listedepart::fetch_all ", LOG_DEBUG );
		...
		if ($resql) {
			...
			if ($num) {
				while ( $i < $num ) {
					...
					$line->NbEnf =  $obj->NbEnf;
					$line->NbAdulte =  $obj->NbAdulte;
					$line->NbAgeNonRensg =  $obj->NbAgeNonRensg;
...
if (! empty($arrayfields['ListBull']['checked']))		
	print_liste_field_titre ( $langs->trans ( "ListBull" ), $_SERVEUR ['PHP_SELF'], '', '', $arg_url, '', $sortfield, $sortorder );
	print '<td></td>';
	print "</tr>\n";
	...
	//			print '<td>' .($line->type_session ? $langs->trans ( 'AgfFormTypeSessionInter' ) : $langs->trans ( 'AgfFormTypeSessionIntra' )). '</td>';
			...
			if (! empty($arrayfields['Statut']['checked']))	{
				...
				print  '<td><font '.$colortexte.'>'  .$texte . '</td>';
					...
					//$line->NbAgeNonRensg =  $obj->NbAgeNonRensg;
			if (! empty($arrayfields['NbEnfant']['checked']))	{
					print '<td align=center>'.$line->NbEnf.'</td>';
			}
			if (! empty($arrayfields['NbAdulte']['checked']))	{
					print '<td align=center>'.$line->NbAdulte;
					if ($line->NbAgeNonRensg>0)  	print info_admin($langs->trans('AgeNonRenseigne'), $line->NbAgeNonRensg);
					print '</td>';
			}
			if (! empty($arrayfields['ListBull']['checked']))  {
					$listBull = $wfdep->html_chercheBullDepart($line->rowid, array("s.rowid"=>$line->rowid));
					print '<td >' .  $listBull . '</td>';
			}
